//+------------------------------------------------------------------+
//|                  AdvancedMartingale_EA_v6.151.mq5                |
//|                                  Copyright 2025, MetaQuotes Ltd. |
//|                                             https://www.mql5.com |
//+------------------------------------------------------------------+
#property copyright "Copyright 2025, MetaQuotes Ltd."
#property link      "https://www.mql5.com"
#property version   "6.151"
#property description "高级自适应马丁策略 v6.151 (无临时止盈版本)"
#property description "新增: 趋势反转识别 + 防单边机制 + 智能手数 + 加速检测"
...
//--- 输入参数
input group "=== 基础参数 ==="
input double   InitialLot         = 0.03;     // 初始手数
...
input double   Weight_250pts      = 1.2;      // 波动超250点权重系数
input double   Weight_300pts      = 1.4;      // 波动超300点权重系数
input double   Weight_350pts      = 1.6;      // 波动超350点权重系数
...
input group "=== 第六单反向对冲机制 ==="
input bool     EnableSixthOrderReverse = true;       // 启用第六单反向对冲
input int      SixthOrderReversePoints = 2200;       // 第六单反向触发点数(>2200点)

input group "=== 日志设置 ==="
input bool     EnableFileLog      = true;     // 启用文件日志
input string   LogFileName        = "EA_v6.151_Log.txt"; // 日志文件名
...
//--- 趋势逆向防御状态
bool reverseDefenseTriggered = false;     // 逆向防御是否已触发
ulong reverseDefenseTicket = 0;           // 逆向防御单票据号
double reverseDefenseVolume = 0;          // 逆向防御单手数
datetime firstOrderOpenTime = 0;          // 本轮第一单开仓时间
double firstOrderOpenPrice = 0;           // 本轮第一单开仓价格

//--- 第六单反向对冲状态
bool sixthOrderReverseTriggered = false; // 第六单反向对冲是否已触发
ulong sixthOrderReverseTicket = 0;       // 第六单反向对冲单票据号
double sixthOrderReverseVolume = 0;      // 第六单反向对冲单手数
...
{
   Print("========================================");
   Print("高级马丁策略 v6.151 已启动 (无临时止盈版本)");
   Print("品种: ", _Symbol);
   Print("新功能: 趋势反转识别 + 防单边10000点 + 智能手数");
   Print("新功能: 单边加速检测 + 6单休眠");
   Print("波动系数: 250→1.2, 300→1.4, 350→1.6, 400→2.0");
   Print("初始手数: 0.03");
   Print("========================================");
   ...
   WriteLog("========================================");
   WriteLog("EA v6.151 启动 (无临时止盈版本)");
   WriteLog("品种: " + _Symbol);
   WriteLog("账户: " + IntegerToString(AccountInfoInteger(ACCOUNT_LOGIN)));
   WriteLog("杠杆: " + DoubleToString(Leverage, 0) + "倍");
   WriteLog("初始手数: " + DoubleToString(InitialLot, 2));
   WriteLog("--- 新功能启用状态 ---");
   WriteLog("趋势反转识别: " + (EnableReversal ? "启用" : "禁用"));
   WriteLog("防单边机制: " + (EnableAntiExtremeMove ? "启用" : "禁用"));
   WriteLog("临时止盈: 已移除");
   WriteLog("趋势加速检测: " + (EnableTrendAcceleration ? "启用" : "禁用"));
   WriteLog("--- 分级休眠控制 ---");
   WriteLog("分级休眠: " + (EnableGradedSleep ? "启用" : "禁用"));
   if(EnableGradedSleep)
   {
      WriteLog(StringFormat("4单休眠%d分钟, 5单休眠%d分钟, ≥6单休眠%d分钟", 
                           Sleep4OrdersMinutes, Sleep5OrdersMinutes, Sleep6OrdersMinutes));
   }
   WriteLog("--- 基础时间控制 ---");
   ...
   WriteLog("--- 趋势逆向防御机制 ---");
   WriteLog("逆向防御: " + (EnableReverseDefense ? "启用" : "禁用"));
   if(EnableReverseDefense)
   {
      WriteLog(StringFormat("最小加单数: %d, 触发点数: %d", ReverseMinOrders, ReverseTotalPoints));
      WriteLog(StringFormat("K线逆向比例: %.1f%%, 对冲倍数: %.1f", ReverseKlineRatio, ReverseHedgeLotMultiplier));
      WriteLog(StringFormat("盈利目标: $%.2f", ReverseDefenseProfit));
   }
   WriteLog("--- 第六单反向对冲机制 ---");
   WriteLog("第六单反向对冲: " + (EnableSixthOrderReverse ? "启用" : "禁用"));
   if(EnableSixthOrderReverse)
   {
      WriteLog(StringFormat("触发点数: %d点, 对冲倍数: 2.0倍", SixthOrderReversePoints));
   }
   WriteLog("========================================");
...
void OnTick()
{
   ...
   if(marginProfitRate >= targetMarginRate && totalOrders > 0)
   {
      ...
      // 记录本轮最大订单数
      int currentRoundMaxOrders = totalOrders;
      ...
      if(EnableGradedSleep)
      {
         ...
         {
            sleepMinutes = Sleep6OrdersMinutes;
            sleepLevel = "≥6单";
         }
         else if(currentRoundMaxOrders == 5)
         {
            sleepMinutes = Sleep5OrdersMinutes;
            sleepLevel = "5单";
         }
         else if(currentRoundMaxOrders == 4)
         {
            sleepMinutes = Sleep4OrdersMinutes;
            sleepLevel = "4单";
         }
      }
      ...
      {
         WriteLog("========================================");
         WriteLog("上一轮订单数: " + IntegerToString(currentRoundMaxOrders));
         WriteLog(sleepLevel + ",进入休眠" + IntegerToString(sleepMinutes) + "分钟");
         WriteLog(StringFormat("分级休眠: 4单=%d分钟, 5单=%d分钟, ≥6单=%d分钟", 
                              Sleep4OrdersMinutes, Sleep5OrdersMinutes, Sleep6OrdersMinutes));
         WriteLog("========================================");
         ...
         // 保存当前轮数据供下轮使用
         lastRoundMaxOrders = currentRoundMaxOrders;
      }
   ...
   if(martingaleEnabled && firstOrderOpened && totalOrders > 0)
   {
      ...
      {
         // 先尝试触发防御机制
         if(!reverseDefenseTriggered && !sixthOrderReverseTriggered)
         {
            ExecuteReverseDefense();
         }
         
         // 检查防御盈利目标
         CheckReverseDefenseProfit();
         
         // 关键修改：如果防御机制已触发，不再执行原有交易逻辑
         if(reverseDefenseTriggered || sixthOrderReverseTriggered)
         {
            WriteLog("逆向防御已触发，暂停原有交易逻辑，等待盈利目标达成");
            return;  // 直接返回，不执行ExecuteStrategy()
         }
      }
...
int AnalyzeFirstOrderDirection()
{
   ...
   // 计算趋势判断的可信度
   double trendConfidence = MathAbs(trendScore);
   
   string msg = StringFormat("第一单方向: 1m=%.3f | 5m=%.3f | 15m=%.3f | 1h=%.3f | 趋势得分=%.3f",
                             trend1m, trend5m, trend15m, trend1h, trendScore);
   WriteLog(msg);
   WriteLog(StringFormat("反转检测: 方向=%d, 得分=%.3f, 可信度=%.3f", 
                         reversalDirection, reversalScore, trendConfidence));
   
   // 决策逻辑：优先反转信号，其次趋势判断
   if(EnableReversal && reversalScore > 0.7)
   {
      // 反转信号可信度高，使用反转方向
      WriteLog("采用反转信号方向: " + (reversalDirection > 0 ? "看多" : "看空"));
      return reversalDirection;
   }
   else if(trendConfidence > 0.5)
   {
      // 趋势判断可信度高，使用趋势方向
      WriteLog("采用趋势判断方向: " + (trendScore > 0 ? "看多" : "看空"));
      return (trendScore > 0 ? 1 : -1);
   }
   else if(EnableReversal && reversalScore > 0.5)
   {
      // 趋势不明确但有一定反转信号，使用反转方向
      WriteLog("采用反转信号方向（次优）: " + (reversalDirection > 0 ? "看多" : "看空"));
      return reversalDirection;
   }
   else
   {
      // 都不明确，使用趋势方向或默认看涨
      WriteLog("方向不明确，使用默认方向");
      return (trendScore > 0.1 ? 1 : (trendScore < -0.1 ? -1 : 1));
   }
}
...
bool OpenFirstOrder()
{
   ...
   // 使用智能手数（第一单）
   double firstLotSize = CalculateSmartVolume(0);
   WriteLog("首单使用智能手数: " + DoubleToString(firstLotSize, 2));
   
   if(OpenOrder(orderType, firstLotSize, "首单"))
   {
...
void ExecuteStrategy()
{
   ...
   // 检测趋势加速并调整间隔
   if(EnableTrendAcceleration && currentStepPoints < 200)
   {
      double accelerationMultiplier = DetectTrendAcceleration(currentStepPoints);
      if(accelerationMultiplier > 1.0)
      {
         double originalStep = currentStepPoints;
         currentStepPoints = currentStepPoints * accelerationMultiplier;
         WriteLog(StringFormat("趋势加速调整: 原间隔=%.0f点, 调整后=%.0f点, 系数=%.2f",
                              originalStep, currentStepPoints, accelerationMultiplier));
      }
   }
...
{
   // 如果防单边机制已触发，不再开新的原方向订单
   if(antiExtremeActive)
   {
      WriteLog("防单边机制已触发，停止开新的原方向订单");
      return;
   }
   ...
   else if(firstDirection == POSITION_TYPE_SELL && totalSellOrders < MaxOrders && lastSellPrice > 0)
   {
      ...
      {
         // 使用智能手数
         double newVol = CalculateSmartVolume(totalSellOrders);
...
bool OpenOrder(ENUM_ORDER_TYPE orderType, double volume, string comment)
{
   ...
   if(totalOrders == 0)
   {
      ...
      firstOrderOpenTime = currentTime;  // 记录第一单开仓时间（用于逆向防御）
      firstOrderOpenPrice = price;        // 记录第一单开仓价格（用于逆向防御）
   }
...
void CloseAllOrders()
{
   ...
   // 重置防单边机制状态
   antiExtremeActive = false;
   antiExtremeTicket = 0;
   
   // 重置逆向防御状态
   reverseDefenseTriggered = false;
   reverseDefenseTicket = 0;
   reverseDefenseVolume = 0;
   firstOrderOpenTime = 0;
   firstOrderOpenPrice = 0;
   
   // 重置第六单反向对冲状态
   sixthOrderReverseTriggered = false;
   sixthOrderReverseTicket = 0;
   sixthOrderReverseVolume = 0;
...
datetime CalculateSleepEndTime(string reason)
{
   ...
   // 每日收盘前 - 休眠35分钟（收盘前20分钟开始，收盘后15分钟恢复）
   if(StringFind(reason, "每日收盘") >= 0)
   {
      // 从当前时间（收盘前20分钟）休眠35分钟
      // 04:40开始休眠 → 05:15恢复
      return currentTime + 35 * 60;
   }
   
   // 市场盘切换窗口 - 休眠25分钟（切换前15分钟开始，切换后10分钟恢复）
   if(StringFind(reason, "盘开盘时间窗口") >= 0)
   {
      // 从当前时间（切换前15分钟）休眠25分钟
      // 例如：14:45开始休眠 → 15:10恢复（欧洲盘15:00开）
...
void CalculateNext24hEvents()
{
   ...
   for(datetime checkTime = currentTime; checkTime <= endTime; checkTime += 60)
   {
      ...
      if(checkMinutes == beforeCloseStart && checkTime > currentTime)
      {
         ...
         eventList[eventCount].description = "[每日收盘] 休眠开始(休35分钟)";
         eventCount++;
      }
      
      // 检查休眠恢复时间（收盘前20分钟+35分钟=收盘后15分钟）
      int sleepRecoveryMinutes = beforeCloseStart + 35;
      if(sleepRecoveryMinutes >= 24 * 60) sleepRecoveryMinutes -= 24 * 60;
      if(checkMinutes == sleepRecoveryMinutes && checkTime > currentTime)
      {
         eventList[eventCount].time = checkTime;
         eventList[eventCount].description = "[每日收盘] 休眠恢复(收盘后15分)";
         eventCount++;
      }
      ...
      {
         eventList[eventCount].time = checkTime;
         eventList[eventCount].description = "[亚洲盘] 休眠开始(休25分钟)";
         eventCount++;
      }
      int asiaRecovery = beforeAsiaStart + 25;
      if(asiaRecovery >= 24 * 60) asiaRecovery -= 24 * 60;
      if(checkMinutes == asiaRecovery && checkTime > currentTime)
      {
         eventList[eventCount].time = checkTime;
         eventList[eventCount].description = "[亚洲盘] 休眠恢复(开盘后10分)";
         eventCount++;
      }
      ...
      {
         eventList[eventCount].time = checkTime;
         eventList[eventCount].description = "[欧洲盘] 休眠开始(休25分钟)";
         eventCount++;
      }
      int europeRecovery = beforeEuropeStart + 25;
      if(europeRecovery >= 24 * 60) europeRecovery -= 24 * 60;
      if(checkMinutes == europeRecovery && checkTime > currentTime)
      {
         eventList[eventCount].time = checkTime;
         eventList[eventCount].description = "[欧洲盘] 休眠恢复(开盘后10分)";
         eventCount++;
      }
      ...
      {
         eventList[eventCount].time = checkTime;
         eventList[eventCount].description = "[美国盘] 休眠开始(休25分钟)";
         eventCount++;
      }
      int usRecovery = beforeUSStart + 25;
      if(usRecovery >= 24 * 60) usRecovery -= 24 * 60;
      if(checkMinutes == usRecovery && checkTime > currentTime)
      {
         eventList[eventCount].time = checkTime;
         eventList[eventCount].description = "[美国盘] 休眠恢复(开盘后10分)";
         eventCount++;
      }
...
void DetectTrendReversal()
{
   ...
   // 信号1: K线形态反转 (权重30%)
   double candleReversalScore = DetectCandleReversal();
   
   // 信号2: 成交量背离 (权重20%)
   double volumeDivergenceScore = DetectVolumeDivergence();
   
   // 信号3: 动量指标背离 (权重25%)
   double momentumDivergenceScore = DetectMomentumDivergence();
   
   // 信号4: 价格超买超卖 (权重25%)
   double overboughtOversoldScore = DetectOverboughtOversold();
   
   // 综合得分
   double totalScore = candleReversalScore * 0.30 + 
                      volumeDivergenceScore * 0.20 + 
                      momentumDivergenceScore * 0.25 + 
                      overboughtOversoldScore * 0.25;
   
   reversalScore = MathAbs(totalScore);
   
   // 判断反转方向
   if(totalScore > ReversalThreshold)
   {
      reversalDirection = 1;  // 看涨反转
      WriteLog(StringFormat("检测到看涨反转信号! 得分=%.3f (阈值=%.3f)", totalScore, ReversalThreshold));
   }
   else if(totalScore < -ReversalThreshold)
   {
      reversalDirection = -1;  // 看跌反转
      WriteLog(StringFormat("检测到看跌反转信号! 得分=%.3f (阈值=-%.3f)", totalScore, ReversalThreshold));
   }
   else
   {
      reversalDirection = 0;  // 无明确反转
   }
}
...
double DetectCandleReversal()
{
   ...
   if(lowerShadow1 > body1 * 2 && upperShadow1 < body1 * 0.3)
   {
      score += 0.5;  // 锤子线
   }
   
   // 吞没形态检测
   if(close2 < open2 && close0 > open0)  // 前阴后阳
   {
      if(close0 > open2 && open0 < close2)  // 阳线吞没阴线
      {
         score += 0.7;  // 看涨吞没
      }
   }
   else if(close2 > open2 && close0 < open0)  // 前阳后阴
   {
      if(close0 < open2 && open0 > close2)  // 阴线吞没阳线
      {
         score -= 0.7;  // 看跌吞没
      }
   }
   ...
   {
      score += (close0 > open0 ? 0.3 : -0.3);  // 十字星后看方向
   }
...
{
   // 简化版本：通过波动率变化判断成交量背离
   double range0 = iHigh(_Symbol, PERIOD_M5, 0) - iLow(_Symbol, PERIOD_M5, 0);
   double range1 = iHigh(_Symbol, PERIOD_M5, 1) - iLow(_Symbol, PERIOD_M5, 1);
   double range2 = iHigh(_Symbol, PERIOD_M5, 2) - iLow(_Symbol, PERIOD_M5, 2);
...
double DetectMomentumDivergence()
{
   ...
   double close9 = iClose(_Symbol, PERIOD_M5, 9);
   
   double momentum1 = close0 - close3;
   double momentum2 = close3 - close6;
   double momentum3 = close6 - close9;
   
   double score = 0;
   
   // 价格上涨但动量递减 (看跌背离)
   if(close0 > close3 && close3 > close6)
   {
      if(MathAbs(momentum1) < MathAbs(momentum2))
      {
         score -= 0.7;
      }
   }
   // 价格下跌但动量递减 (看涨背离)
   else if(close0 < close3 && close3 < close6)
   {
      if(MathAbs(momentum1) < MathAbs(momentum2))
      {
         score += 0.7;
...
double DetectOverboughtOversold()
{
   ...
   {
      double change = iClose(_Symbol, PERIOD_M5, i-1) - iClose(_Symbol, PERIOD_M5, i);
      if(change > 0)
         gains += change;
      else
...
{
   if(!EnableAntiExtremeMove || totalOrders < AntiExtremeStartOrder)
      return;
   ...
   if(antiExtremeActive)
   {
      ...
      if(totalProfit >= AntiExtremeProfitTarget)
      {
         WriteLog("========================================");
         WriteLog("防单边机制: 整体盈利达到$" + DoubleToString(totalProfit, 2));
         WriteLog("立即全局平仓（不休眠）");
         WriteLog("========================================");
   ...
   ENUM_POSITION_TYPE lastOrderType = -1;
   ...
   for(int i = PositionsTotal() - 1; i >= 0; i--)
   {
      if(PositionSelectByTicket(PositionGetTicket(i)))
      {
            ...
            PositionGetInteger(POSITION_MAGIC) == MagicNumber)
         {
            ...
            lastOrderType = (ENUM_POSITION_TYPE)PositionGetInteger(POSITION_TYPE);
            break;  // 取第一个找到的（最后开的单）
         }
      }
   }
   
   if(lastOrderPrice == 0) return;
   
   double currentPrice = (lastOrderType == POSITION_TYPE_BUY) ? 
                        SymbolInfoDouble(_Symbol, SYMBOL_BID) : 
                        SymbolInfoDouble(_Symbol, SYMBOL_ASK);
   double point = SymbolInfoDouble(_Symbol, SYMBOL_POINT);
   
   double priceDiff = 0;
   if(lastOrderType == POSITION_TYPE_BUY)
   {
      priceDiff = (lastOrderPrice - currentPrice) / point;  // 多单继续下跌
   }
   else
   {
      priceDiff = (currentPrice - lastOrderPrice) / point;  // 空单继续上涨
   }
   
   if(priceDiff >= AntiExtremePoints)
   {
      WriteLog("========================================");
      WriteLog("触发防单边机制!");
      WriteLog("价格偏离第" + IntegerToString(totalOrders) + "单超过" + IntegerToString(AntiExtremePoints) + "点");
      WriteLog("开反向双倍手数单进行对冲");
      WriteLog("========================================");
      
      // 计算第8单手数
      double eighthOrderVolume = 0;
      int orderCount = 0;
      ...
      for(int i = PositionsTotal() - 1; i >= 0; i--)
      {
         if(PositionSelectByTicket(PositionGetTicket(i)))
         {
            ...
            {
               orderCount++;
               if(orderCount == 1)  // 最后一单（第8单或更多）
               {
                  eighthOrderVolume = PositionGetDouble(POSITION_VOLUME);
                  break;
               }
            }
         }
      ...
      double antiVolume = eighthOrderVolume * 2.0;
      
      // 开反向单
      ENUM_ORDER_TYPE antiOrderType = (lastOrderType == POSITION_TYPE_BUY) ? 
                                      ORDER_TYPE_SELL : ORDER_TYPE_BUY;
      
      if(OpenOrder(antiOrderType, antiVolume, "防单边对冲"))
      {
         // 记录反向单ticket
         for(int i = PositionsTotal() - 1; i >= 0; i--)
         {
            ...
            if(PositionSelectByTicket(ticket))
            {
               if(PositionGetString(POSITION_SYMBOL) == _Symbol && 
                  PositionGetInteger(POSITION_MAGIC) == MagicNumber &&
                  PositionGetString(POSITION_COMMENT) == "防单边对冲")
               {
                  antiExtremeTicket = ticket;
                  antiExtremeActive = true;
                  WriteLog("防单边对冲单已开: Ticket#" + IntegerToString(ticket) + 
                          ", 手数=" + DoubleToString(antiVolume, 2));
                  break;
               }
            }
         }
...
//+------------------------------------------------------------------+
//| 计算智能手数 (第1单0.03, 第2单0.02, 第3单0.04, 后续翻倍)         |
//+------------------------------------------------------------------+
double CalculateSmartVolume(int orderIndex)
{
   ...
   else if(orderIndex == 1)  // 第二单
   {
      volume = 0.02;
   }
   else if(orderIndex == 2)  // 第三单
   {
      volume = 0.04;
   }
   else  // 第四单及以后，都是前一单的两倍
   {
      // 获取前一单的手数
      double prevVolume = 0;
      int count = 0;
      ...
      for(int i = PositionsTotal() - 1; i >= 0; i--)
      {
         if(PositionSelectByTicket(PositionGetTicket(i)))
         {
            ...
            {
               count++;
               if(count == 1)  // 最后一单就是前一单
               {
                  prevVolume = PositionGetDouble(POSITION_VOLUME);
                  break;
               }
            }
         }
      ...
      else
      {
         ...
         if(orderIndex == 3)
            ...
            volume = 0.08 * MathPow(2, orderIndex - 3);
      }
   }
...
//| 检测逆向单边趋势加速                                               |
//+------------------------------------------------------------------+
double DetectTrendAcceleration(double stepPoints)
{
   if(!EnableTrendAcceleration || stepPoints >= 200)
      return 1.0;  // 不需要放大
   ...
   double close5 = iClose(_Symbol, PERIOD_M1, AccelerationLookback);
   
   double point = SymbolInfoDouble(_Symbol, SYMBOL_POINT);
   
   // 计算5分钟总变化（带方向）
   double totalChange = close0 - close5;
   double totalChangePoints = totalChange / point;
   
   // 判断趋势方向
   int trendDirection = 0;
   if(totalChangePoints > 50)  // 向上趋势，至少50点
      trendDirection = 1;  // 看涨
   else if(totalChangePoints < -50)  // 向下趋势，至少50点
      trendDirection = -1;  // 看跌
   else
      return 1.0;  // 趋势不明显，不放大
   
   // 检查趋势方向是否逆着本轮开单方向
   bool isCounterTrend = false;
   if(firstDirection == POSITION_TYPE_BUY && trendDirection == -1)
   {
      // 本轮做多，但行情向下
      isCounterTrend = true;
   }
   else if(firstDirection == POSITION_TYPE_SELL && trendDirection == 1)
   {
      // 本轮做空，但行情向上
      isCounterTrend = true;
   }
   ...
   // 是逆向趋势，检测加速度
   // 计算平均每分钟变化
   double avgChangePerMin = MathAbs(totalChangePoints) / AccelerationLookback;
   
   // 计算最近1分钟的变化
   double close1min = iClose(_Symbol, PERIOD_M1, 1);
   double recentChange = MathAbs(close0 - close1min) / point;
   
   // 计算加速比
   double accelerationRatio = (avgChangePerMin > 0) ? recentChange / avgChangePerMin : 1.0;
   ...
   if(accelerationRatio >= 1.5)
   {
      ...
      // 基础放大系数 = 1.2
      // 根据加速比调整，最大2.0
      double multiplier = AccelerationMinMultiplier + 
                         (accelerationRatio - 1.5) * 
                         (AccelerationMaxMultiplier - AccelerationMinMultiplier) / 2.0;
      
      if(multiplier > AccelerationMaxMultiplier)
         multiplier = AccelerationMaxMultiplier;
      if(multiplier < AccelerationMinMultiplier)
         multiplier = AccelerationMinMultiplier;
      
      string trendDesc = (trendDirection == 1) ? "向上" : "向下";
      string positionDesc = (firstDirection == POSITION_TYPE_BUY) ? "做多" : "做空";
      
      WriteLog(StringFormat("检测到逆向单边趋势加速! 本轮%s, 行情%s", positionDesc, trendDesc));
      WriteLog(StringFormat("5分钟变化=%.0f点, 平均=%.0f点/分, 最近1分=%.0f点", 
                           MathAbs(totalChangePoints), avgChangePerMin, recentChange));
      WriteLog(StringFormat("加速比=%.2f, 放大系数=%.2f", accelerationRatio, multiplier));
      
      return multiplier;
   }
   else
   {
      // 虽然是逆向趋势，但没有加速，使用最小放大系数
      WriteLog(StringFormat("检测到逆向趋势但无明显加速，使用最小放大系数%.2f", 
                           AccelerationMinMultiplier));
      return AccelerationMinMultiplier;
   }
}
...
//| 检测趋势逆向防御条件                                              |
//+------------------------------------------------------------------+
bool CheckReverseDefenseCondition()
{
   // 如果未启用或已触发，直接返回
   if(!EnableReverseDefense || reverseDefenseTriggered)
      return false;
   
   // 特殊检查：第六单反向对冲机制（优先级最高）
   if(EnableSixthOrderReverse && !sixthOrderReverseTriggered && totalOrders == 6)
   {
      // 获取第六单的价格
      double sixthOrderPrice = 0;
      for(int i = PositionsTotal() - 1; i >= 0; i--)
      {
         if(PositionSelectByTicket(PositionGetTicket(i)))
         {
            if(PositionGetString(POSITION_SYMBOL) == _Symbol && 
               PositionGetInteger(POSITION_MAGIC) == MagicNumber)
            {
               sixthOrderPrice = PositionGetDouble(POSITION_PRICE_OPEN);
               break;  // 取第一个找到的（最后开的单）
            }
         }
      }
      
      if(sixthOrderPrice > 0)
      {
         double priceGap = MathAbs(sixthOrderPrice - firstOrderOpenPrice) / _Point;
         
         if(priceGap > SixthOrderReversePoints)
         {
            WriteLog("========================================");
            WriteLog("触发第六单反向对冲条件！");
            WriteLog(StringFormat("第六单价格: %.5f, 第一单价格: %.5f", sixthOrderPrice, firstOrderOpenPrice));
            WriteLog(StringFormat("价差: %.1f点 (> %d点)", priceGap, SixthOrderReversePoints));
            WriteLog("========================================");
   ...
   // 条件1: 加单数必须 >= 5
   if(totalOrders < ReverseMinOrders)
      return false;
   
   // 条件2: 计算从第一单到当前的总间隔
   double currentPrice = 0;
   if(firstDirection == POSITION_TYPE_BUY)
      currentPrice = SymbolInfoDouble(_Symbol, SYMBOL_ASK);
   else if(firstDirection == POSITION_TYPE_SELL)
      currentPrice = SymbolInfoDouble(_Symbol, SYMBOL_BID);
   else
      return false;
   
   double totalPointsGap = MathAbs(currentPrice - firstOrderOpenPrice) / _Point;
   
   if(totalPointsGap <= ReverseTotalPoints)
      return false;
   
   // 条件3: 分析从第一单开仓到当前的K线走势
   // 检测是否80%的K线在逆着开单方向发展
   if(!AnalyzeKlineReverseTrend())
      return false;
   
   WriteLog("========================================");
   WriteLog("触发趋势逆向防御条件！");
   WriteLog(StringFormat("当前加单数: %d (>= %d)", totalOrders, ReverseMinOrders));
   WriteLog(StringFormat("总间隔: %.1f点 (> %d)", totalPointsGap, ReverseTotalPoints));
   WriteLog("K线分析: 80%以上K线逆向发展且有加速趋势");
   WriteLog("========================================");
...
//| 分析K线逆向趋势                                                   |
//+------------------------------------------------------------------+
bool AnalyzeKlineReverseTrend()
{
   // 获取从第一单开仓时间到当前的所有1分钟K线
   datetime currentTime = TimeCurrent();
   int totalBars = (int)((currentTime - firstOrderOpenTime) / 60);  // 1分钟K线数量
   
   if(totalBars < 5)  // 至少需要5根K线才能判断
      return false;
   ...
   int copied = CopyRates(_Symbol, PERIOD_M1, 0, totalBars, rates);
   if(copied < 5)
      return false;
   ...
   // 用于检测加速趋势的变量
   double totalMove = 0;
   double recentMove = 0;  // 最近1/3时间的移动
   int recentBars = totalBars / 3;
   ...
   for(int i = 0; i < copied; i++)
   {
      ...
      if(firstDirection == POSITION_TYPE_BUY)
      {
         // 买单方向，如果K线收盘价低于开盘价，则为逆向
         if(close < open)
            isReverse = true;
         
         // 统计移动幅度
         double move = open - close;
         totalMove += move;
         if(i < recentBars)
            recentMove += move;
      }
   ...
   // 计算逆向K线比例
   double reverseRatio = (double)reverseCount / totalCount * 100.0;
   
   // 检测加速趋势：最近1/3时间的平均移动 > 整体平均移动的1.2倍
   double avgMove = totalMove / totalCount;
   double recentAvgMove = recentMove / recentBars;
   bool hasAcceleration = (recentAvgMove > avgMove * 1.2);
   
   WriteLog(StringFormat("K线分析: 总K线数=%d, 逆向K线数=%d, 逆向比例=%.1f%%, 需要>%.1f%%",
            totalCount, reverseCount, reverseRatio, ReverseKlineRatio));
   WriteLog(StringFormat("加速检测: 整体均速=%.5f, 近期均速=%.5f, 加速=%s",
            avgMove, recentAvgMove, hasAcceleration ? "是" : "否"));
   
   // 判断条件：逆向比例 >= 阈值 且 有加速趋势
   return (reverseRatio >= ReverseKlineRatio && hasAcceleration);
}

//+------------------------------------------------------------------+
//| 执行逆向防御开单                                                  |
//+------------------------------------------------------------------+
void ExecuteReverseDefense()
{
   if(!CheckReverseDefenseCondition())
      return;
   
   // 检查是否是第六单反向对冲
   bool isSixthOrderReverse = (EnableSixthOrderReverse && !sixthOrderReverseTriggered && totalOrders == 6);
   ...
   if(isSixthOrderReverse)
   {
      // 第六单反向对冲：使用第六单手数的双倍
      reverseVolume = lastVolume * 2.0;  // 固定双倍
      comment = "第六单反向对冲[2.0倍]";
      
      WriteLog("========================================");
      WriteLog("执行第六单反向对冲开单");
      WriteLog(StringFormat("第六单手数: %.2f, 反向单手数: %.2f", lastVolume, reverseVolume));
   }
   else
   {
      // 常规逆向防御：使用配置的倍数
      reverseVolume = lastVolume * ReverseHedgeLotMultiplier;
      comment = StringFormat("逆向防御[%.2f倍]", ReverseHedgeLotMultiplier);
      
      WriteLog("========================================");
      WriteLog("执行逆向防御开单");
      WriteLog(StringFormat("最后加仓单手数: %.2f, 逆向单手数: %.2f", lastVolume, reverseVolume));
   }
   
   // 确定逆向单方向
   ENUM_ORDER_TYPE reverseType;
   if(firstDirection == POSITION_TYPE_BUY)
      reverseType = ORDER_TYPE_SELL;
   else
      reverseType = ORDER_TYPE_BUY;
   
   WriteLog(StringFormat("原方向: %s, 逆向方向: %s", 
            firstDirection == POSITION_TYPE_BUY ? "买" : "卖",
            reverseType == ORDER_TYPE_BUY ? "买" : "卖"));
   WriteLog("========================================");
   
   if(OpenOrder(reverseType, reverseVolume, comment))
   {
      if(isSixthOrderReverse)
      {
         sixthOrderReverseTriggered = true;
         sixthOrderReverseVolume = reverseVolume;
         WriteLog("第六单反向对冲开仓成功！");
      }
      else
      {
         reverseDefenseTriggered = true;
         reverseDefenseVolume = reverseVolume;
         WriteLog("逆向防御单开仓成功！");
      }
   }
   else
...
//| 检查逆向防御盈利目标                                              |
//+------------------------------------------------------------------+
void CheckReverseDefenseProfit()
{
   if(!reverseDefenseTriggered && !sixthOrderReverseTriggered)
      return;
   ...
   // 如果达到目标盈利，立即平仓
   if(totalProfit >= ReverseDefenseProfit)
   {
      WriteLog("========================================");
      if(sixthOrderReverseTriggered)
      {
         WriteLog("第六单反向对冲盈利目标已达成！");
      }
      else
      {
         WriteLog("逆向防御盈利目标已达成！");
      }
      WriteLog(StringFormat("当前盈利: $%.2f (>= $%.2f)", totalProfit, ReverseDefenseProfit));
      WriteLog("执行一键平仓（平掉当前轮所有单子）");
      WriteLog("========================================");
      ...
      // 进入休眠状态30分钟
      isSleeping = true;
      sleepReason = "第六单反向对冲平仓后休眠";
      sleepStartTime = TimeCurrent();
      
      // 处理休眠时间重叠：如果已有休眠时间，使用较长的休眠时间
      datetime newSleepEndTime = TimeCurrent() + 30 * 60;  // 30分钟
      if(isSleeping && sleepEndTime > TimeCurrent())
      {
         // 如果当前休眠时间更长，保持当前休眠时间
         if(sleepEndTime > newSleepEndTime)
         {
            WriteLog("检测到休眠时间重叠，保持当前休眠时间");
            WriteLog(StringFormat("当前休眠剩余: %d分钟", (int)((sleepEndTime - TimeCurrent()) / 60)));
         }
         else
         {
            // 如果新的休眠时间更长，使用新的休眠时间
            sleepEndTime = newSleepEndTime;
            WriteLog("检测到休眠时间重叠，使用较长的休眠时间");
         }
      }
      else
      {
      ...
      WriteLog("========================================");
      WriteLog("第六单反向对冲触发后休眠机制");
      WriteLog("休眠时长: 30分钟");
      WriteLog(StringFormat("休眠结束时间: %s", TimeToString(sleepEndTime)));
      WriteLog("休眠期间暂停新一轮自动交易，直到休眠结束");
      WriteLog("========================================");
      ...
      // 更新模式按钮状态
      if(ObjectFind(0, btnMode) >= 0)
      {
         ObjectSetString(0, btnMode, OBJPROP_TEXT, "休眠30m");
         ObjectSetInteger(0, btnMode, OBJPROP_BGCOLOR, clrRed);
      }
   }
}
