...
int OnInit()
{
   ...
   WriteLog("========================================");
   WriteLog("EA v6.141fix3 启动 (短时间每日休眠版本)");
   WriteLog("品种: " + _Symbol);
   ...
   if(EnableMarketTimeControl)
   {
      ...
      WriteLog(StringFormat("每日收盘: %02d:%02d (前%d分钟禁止，避免过夜费)", DailyCloseHour, DailyCloseMin, BeforeCloseMinutes));
...
void CalculateNext24hEvents()
{
   ...
   // 计算未来24小时内的所有交易事件
   for(int day = 0; day < 2; day++)  // 今天和明天
   {
      for(int hour = 0; hour < 24; hour++)
      {
         for(int min = 0; min < 60; min += 30)  // 每30分钟检查一次
         {
            int checkMinutes = hour * 60 + min;
            datetime checkTime = currentTime + day * 24 * 3600 + (checkMinutes - currentMinutes) * 60;
            
            if(checkTime <= currentTime) continue;
            if(checkTime > currentTime + 24 * 3600) break;
            ...
            if(checkDt.hour * 60 + checkDt.min == beforeCloseStart)
            {
               events += StringFormat("%02d:%02d 每日收盘前%d分钟休眠\n", 
                                    checkDt.hour, checkDt.min, BeforeCloseMinutes);
               eventCount++;
            }
            ...
            if(checkDt.hour * 60 + checkDt.min == afterOpenEnd)
            {
               events += StringFormat("%02d:%02d 每日开盘后%d分钟休眠结束\n", 
                                    checkDt.hour, checkDt.min, AfterOpenMinutes);
               eventCount++;
            }
            ...
            if(checkDt.hour * 60 + checkDt.min == beforeAsiaStart)
            {
               events += StringFormat("%02d:%02d 亚洲盘开盘前%d分钟休眠\n", 
                                    checkDt.hour, checkDt.min, BeforeKeyTimeMinutes);
               eventCount++;
            }
            ...
            if(checkDt.hour * 60 + checkDt.min == beforeEuropeStart)
            {
               events += StringFormat("%02d:%02d 欧洲盘开盘前%d分钟休眠\n", 
                                    checkDt.hour, checkDt.min, BeforeKeyTimeMinutes);
               eventCount++;
            }
            ...
            if(checkDt.hour * 60 + checkDt.min == beforeUSStart)
            {
               events += StringFormat("%02d:%02d 美国盘开盘前%d分钟休眠\n", 
                                    checkDt.hour, checkDt.min, BeforeKeyTimeMinutes);
               eventCount++;
            }
            
            // 检查周五收盘
            if(NoWeekendPositions && checkDt.day_of_week == 5)
            {
               int fridayCloseMinutes = FridayCloseHour * 60 + FridayCloseMin;
               if(checkDt.hour * 60 + checkDt.min == fridayCloseMinutes)
               {
                  events += StringFormat("%02d:%02d 周五收盘休眠\n", 
                                       checkDt.hour, checkDt.min);
                  eventCount++;
               }
            }
