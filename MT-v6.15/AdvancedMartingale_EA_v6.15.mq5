...
#property version   "6.150"
#property description "高级自适应马丁策略 v6.15 (新手数序列版本)"
#property description "优化动态加仓间隔 + 三级优先级系统 + 修正盈利率递减 + 短时间每日休眠避免过夜费"
#property description "新手数序列: 0.03→0.02→0.04→0.08→0.16→0.32→0.64→1.28 (最多8单)"
...
bool OpenFirstOrder()
{
   ...
   // 使用新的手数序列计算函数（第一单）
   double firstVolume = CalculateOrderVolume(1);
   if(OpenOrder(orderType, firstVolume, "首单"))
   {
...
void ExecuteSingleTrendStrategy(double ask, double bid, double point)
{
   if(firstDirection == POSITION_TYPE_BUY && totalBuyOrders < MaxOrders && lastBuyPrice > 0)
   {
      ...
      {
         // 使用新的手数序列计算函数
         double newVol = CalculateOrderVolume(totalBuyOrders + 1);
   ...
   else if(firstDirection == POSITION_TYPE_SELL && totalSellOrders < MaxOrders && lastSellPrice > 0)
   {
      ...
      {
         // 使用新的手数序列计算函数
         double newVol = CalculateOrderVolume(totalSellOrders + 1);
...
//| 返回值：该订单应该使用的手数                                        |
//+------------------------------------------------------------------+
double CalculateOrderVolume(int orderNum)
{
   double volume = 0.0;
   
   // 手数序列：0.03, 0.02, 0.04, 0.08, 0.16, 0.32, 0.64, 1.28
   switch(orderNum)
   {
      case 1:  // 第一单
         volume = 0.03;
      ...
      default:
         // 超出范围，使用初始手数
         volume = InitialLot;
         WriteLog(StringFormat("警告: 订单序号 %d 超出范围(1-8)，使用初始手数 %.2f", orderNum, InitialLot));
         break;
   }
